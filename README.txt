...
Next, we need to secure the device file with a udev rule. Nobody but the superuser/admin should be able to read or write to the device. We use the group "staff" here to limit access, so make sure your user account is in the "staff" group.
...
I only have one Grid+ available, so I don't know if the serial number is actually unique to each device or it could be the same across all devices. You will need to find your serial number and put it into the udev rule yourself.
...
Now we create a new file: /etc/udev/rules.d/90-NZXT-Grid+_v2.rules
...
Now you need to unplug/replug the device again so that udev will apply the new rule.
...
IMPORTANT: The first time you use the controller on a PC after boot, it needs to be "initialized". Run the "gridfan init" command to do this. You only need to initialize the controller once after it boots up, until the next time your reboot your PC or unplug the controller.
...
The controller needs to be initialized/sycned up with the PC each time it boots. I suspect this is needed by the Microchip serial UART controller as part of an auto-speed negotiation mechanism, but I'm not sure.
